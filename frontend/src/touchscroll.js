...
if (typeof window !== 'undefined' && !window.__marcheTouchScroll) {
    window.__marcheTouchScroll = true;
    let actief = false;
    let startY = 0;
    let startScroll = 0;
    let bewogen = false;
    let onderdrukKlik = false;
    const isVeld = (el) => el instanceof HTMLElement && !!el.closest('input, textarea, select');
    function down(e) {
        if (e.pointerType === 'touch')
            return; // echte touch scrolt native
        if (e.button !== 0)
            return;
        if (isVeld(e.target))
            return; // laat tekstvelden met rust
        actief = true;
        bewogen = false;
        startY = e.clientY;
    ...
    function move(e) {
        ...
        if (!bewogen && Math.abs(dy) < 8)
            return; // kleine beweging = klik, niet scrollen
        bewogen = true;
        onderdrukKlik = true;
