...
function ean13Check(body12: string): number {
  let som = 0;
  for (let i = 0; i < 12; i++) { const d = Number(body12[i]); som += i % 2 === 0 ? d : d * 3; }
  return (10 - (som % 10)) % 10;
}
...
function ean13Modules(code: string): string {
  ...
  for (let i = 0; i < 6; i++) bits += (patroon[i] === 'L' ? L : G)[d[1 + i]];
...
export function ean13Svg(code: string): string {
  ...
  for (let i = 0; i < bits.length; i++) {
    if (bits[i] === '1') rects += `<rect x="${quiet + i * mod}" y="0" width="${mod}" height="${hoogte}" fill="#000"/>`;
  }
...
export type WeegEtiketData = {
  naam: string; weegNummer: number; eenheidsprijs: number; gewicht: number; prijs: number; barcode: string;
};

// Opent een printvenster met het etiket (naam, gewicht, prijs, barcode).
export function drukEtiketAf(d: WeegEtiketData) {
  ...
  if (!w) { alert('Sta pop-ups toe om het etiket af te drukken.'); return; }
